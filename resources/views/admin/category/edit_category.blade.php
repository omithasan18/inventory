                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="card card-primary">
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form action="{{route('category.update',$cat_by_id->id)}}" method="post" enctype="multipart/form-data">
                                @method('patch')
                                @csrf
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">Category Title</label>
                                    <input type="text" value="{{$cat_by_id->title}}"name="title" class="form-control" id="exampleInputEmail1" placeholder="Category Title">
                                    </div>
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">Category image</label>
                                        <input type="file" name="cat_img" class="form-control" id="exampleInputEmail1" placeholder="Role Title">
                                        <input type="hidden" value="{{$cat_by_id->image}}" name="old_cat_img" class="form-control" id="exampleInputEmail1">
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <!-- select -->
                                            <div class="form-group">
                                                ...
                                                <select class="custom-select" name="status">
                                                    <option value="1" @php if ($cat_by_id['status'] == 1) { echo "selected"; } @endphp>Active</option>
                                                    <option value="0" @php if ($cat_by_id['status'] == 0) { echo "selected"; } @endphp>Inactive</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
